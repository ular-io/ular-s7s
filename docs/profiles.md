...
- When the folder filter opens, folders already selected in `filter` appear first; selected and unselected groups remain alphabetical. This order is captured at open time and stays stable while the dialog is open, including across live selection changes and search queries. Reopening captures the latest selection state and creates a new order.

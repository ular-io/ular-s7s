...
/// OS cache base directory (`dirs::cache_dir()`, macOS `~/Library/Caches`).
fn os_cache_base() -> PathBuf {
...
/// Demo sandbox root: `<os cache dir>/s7s/demo`. Lives under the OS cache location
/// (not the process cwd) so `s7s demo` never pollutes an arbitrary working directory,
/// and the whole sandbox is disposable generated data.
pub fn demo_root() -> PathBuf {
    os_cache_base().join(APP_NAME).join("demo")
}
...
pub fn cache_path() -> PathBuf {
    if is_demo_mode() {
        demo_root().join("cache").join("index.bin")
    } else {
        os_cache_base().join(APP_NAME).join("index.bin")
    }
}
